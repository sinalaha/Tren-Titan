    ...
    <div className="relative z-10 mx-auto w-full max-w-2xl px-4 py-16 sm:px-6">
      <GlassCard className="space-y-4 p-8" glow="crimson" hover={false}>
        <p className="text-xs uppercase tracking-[0.2em] text-rose-300/90">
          {t("errorPage.dashboard.eyebrow")}
        </p>
        <h1 className="text-2xl font-semibold text-white">{t("errorPage.dashboard.title")}</h1>
        <p className="text-sm text-white/70">
          {error.message || t("errorPage.dashboard.fallback")}
        </p>
        <div className="flex flex-wrap gap-3 pt-2">
          ...
          <Link
            href="/dashboard"
            className="rounded-xl border border-white/20 bg-white/5 px-4 py-2 text-sm text-white/85 transition hover:bg-white/10"
          >
            {t("errorPage.dashboard.reload")}
          </Link>
        </div>
      </GlassCard>
    </div>
